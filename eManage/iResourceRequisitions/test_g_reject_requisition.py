...
def test_reject_requisition():
    with sync_playwright() as p:
        ...
        # Create a requisition first, so there's one to reject
        page.get_by_text("New Requisition", exact=True).first.click()
        time.sleep(2)

        unique_suffix = str(int(time.time()))
        name = f"Automation Reject Position {unique_suffix}"
        ...
        page.get_by_role("button", name="Reject Requisition", exact=True).click()
        time.sleep(1)
        page.get_by_placeholder(re.compile("Explain why this requisition")).fill("Rejected by automated Playwright test.")
        page.get_by_role("button", name="Confirm Reject", exact=True).click()
        wait_for_message(page, re.compile("Resource requisition has been rejected", re.I))
        print("Resource requisition rejected.")
        time.sleep(2)

        page.get_by_text("Rejected", exact=True).first.wait_for(state="visible", timeout=10000)
