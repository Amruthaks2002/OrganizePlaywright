...
def test_aadhar():
    with sync_playwright() as p:
        ...
        #upload a document in the admin's aadhaar card column
        headers = page.locator("thead th")
        aadhaar_index = 0
        for i in range(headers.count()):
            if headers.nth(i).inner_text().strip() == "AADHAAR CARD":
                aadhaar_index = i
                break
        admin_row = page.locator("tbody tr", has_text="Admin User").first
        admin_row.locator("td").nth(aadhaar_index).get_by_role("button", name=" Upload ").click()
        ...
        page.get_by_placeholder("Search by name or email...").fill("admin")
        time.sleep(3)
        page.locator("#view-doc-7-aadhaar").nth(0).click()
        ...
        #delete the document
        page.locator("#view-doc-7-aadhaar").click()
        page.locator("#view-doc-edit").click()
