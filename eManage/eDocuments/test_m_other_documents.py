...
def test_other_document():
    with sync_playwright() as p:
        ...
        # upload a document in the admin's other documents column
        headers = page.locator("thead th")
        other_index = 0
        for i in range(headers.count()):
            if headers.nth(i).inner_text().strip() == "OTHER":
                other_index = i
                break
        admin_row = page.locator("tbody tr", has_text="Admin User").first
        admin_row.locator("td").nth(other_index).get_by_role("button", name=" Upload ").click()
        ...
        page.get_by_placeholder("Search by name or email...").fill("admin")
        time.sleep(3)
        page.locator("#view-doc-7-other").nth(0).click()
        ...
        # delete the document
        page.locator("#view-doc-7-other").click()
        page.locator("#view-doc-edit").click()
