...
def test_work_mode_trends_filters():
    with sync_playwright() as p:
        ...
        work_mode_trends = page.get_by_test_id("sidebar-child-work-mode-trends")
        work_mode_trends.scroll_into_view_if_needed()
        work_mode_trends.click()
        page.wait_for_url(re.compile(r".*/reports/work-mode-trends"))
        time.sleep(1)

        total_work_mode_days = page.locator("text=TOTAL WORK MODE DAYS").locator("xpath=following-sibling::*[1]")
        applications = page.locator("text=WORK MODE APPLICATIONS").locator("xpath=following-sibling::*[1]")
        ...
        default_total = total_work_mode_days.inner_text()
        ...
        apply_filter.click()
        time.sleep(2)
        assert total_work_mode_days.inner_text() != default_total, "Total work mode days did not change after applying the date range filter."
        ...
        expect(total_work_mode_days).to_have_text(default_total)
        ...
        apply_filter.click()
        time.sleep(2)
        assert total_work_mode_days.inner_text() != default_total, "Total work mode days did not change after filtering by employee."
        ...
        apply_filter.click()
        time.sleep(2)
        assert total_work_mode_days.inner_text() != default_total, "Total work mode days did not change after filtering by policy."
        ...
        expect(page.get_by_text("Monthly Work Mode Bar Chart")).to_be_visible()
        page.get_by_role("button", name="Daily", exact=True).click()
        time.sleep(1)
        expect(page.get_by_text("Daily Work Mode Bar Chart")).to_be_visible()
        ...
        apply_filter.click()
        time.sleep(2)
        assert total_work_mode_days.inner_text() != default_total, "Filters did not change the totals before reset."
